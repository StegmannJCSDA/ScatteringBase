...
C Variables:
     ...
     &     S11,S12,S33,S34,WAVEL,X,REFREL_R,REFREL_I
...
C Read calculation parameters from command line argument list
      CALL getarg(1,arg)
      READ(arg,*) REFMED
      IREADEP = 0
      CALL getarg(2,arg)
      ...
      CALL getarg(3,arg)
      READ(arg,*) REFREL_I
      REFREL = CMPLX(REFREL_R,REFREL_I)
      CALL getarg(4,arg)
      READ(arg,*) RAD 
      CALL getarg(5,arg)
      READ(arg,*) WAVEL
C	  Frequency to wavelength conversion [micrometer]:
	  WAVEL = 299792458.0/WAVEL*0.001 
      CALL getarg(6,arg)
      READ(arg,*) NANG0
C      WRITE(*,*)' Enter (real) refractive index of surrounding medium'
C      READ(*,*)REFMED
C      WRITE(*,*)' Wish to enter refr.index or epsilon? (0 or 1)'
C      READ(*,*)IREADEP
C 1000 IF(IREADEP.LE.0)THEN
C          WRITE(*,*)' Enter complex refractive index of sphere'
C          READ(*,*)REFREL
C      ELSE
C          WRITE(*,*)' Enter complex epsilon of sphere'
C          READ(*,*)CXEPS
C          REFREL=SQRT(CXEPS)
C      ENDIF
...
C      WRITE(*,*)' Enter radius'
C      READ(*,*)RAD
C      WRITE(*,*)' Enter wavelength'
C      READ(*,*)WAVEL
C      WRITE(*,*)' Enter NANG = number of angles between 0 and 90'
C      READ(*,*)NANG0
